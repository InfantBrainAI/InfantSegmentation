...
def create_conv(in_channels, out_channels, kernel_size, stride_size, order, num_groups, padding):
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            modules.append(('conv', conv3d(in_channels, out_channels, kernel_size, stride_size,bias, padding)))
        elif char == 'g':
...
class ExtResNetBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride_size=1, order='cge', num_groups=8, **kwargs):
        ...
        if stride_size>1:
            self.conv1 = SingleConv(in_channels, out_channels, kernel_size=4, stride_size=stride_size, order=order, num_groups=num_groups, padding=0)
        else:
        ...
        # residual block
        self.conv2 = SingleConv(out_channels, out_channels, kernel_size=3, stride_size=1,order=order, num_groups=num_groups,padding=1)
